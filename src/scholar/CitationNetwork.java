...
import scholar.corpuses.CSVFactory;
import scholar.corpuses.Corpus;
import scholar.corpuses.DefaultCorpus;
import scholar.reference.Reference;
//import mendeley.MendeleyAPI;
import scholar.ScholarAPI;
//import sql.CybergeoImport;
//import sql.SQLConnection;
import scholar.utils.CSVWriter;
import scholar.utils.GEXFWriter;
import scholar.utils.RISReader;
import scholar.utils.tor.TorPool;
import scholar.utils.tor.TorPoolManager;
...
public class CitationNetwork {
	...
	public static void buildCitationNetwork(String outFile,Corpus existing){
		...
		Corpus progress = new DefaultCorpus();
		for(Reference r:base){
			if(!existing.references.contains(r)){
			  ScholarAPI.fillIdAndCitingRefs(new DefaultCorpus(r));
			  // export
			  new DefaultCorpus(Reference.references.keySet()).csvExport(outFile,true);
			}else{
				System.out.println("Already got this ref : "+r.toString());
			}
			progress.references.add(r);
			progress.csvExport(outFile+"_progress", false);
		}
	}
	...
	public static void buildCitationNetworkFromRefFile(String refFile,String outFile,int depth,String citedFolder){
		...
		if(new File(outFile+"_progress.csv").exists()){
			existing = new CSVFactory(outFile+"_progress.csv").getCorpus();
			//for(Reference r:existing){System.out.println("Existing : "+r.toString());}
		}
