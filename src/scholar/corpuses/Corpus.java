...
public abstract class Corpus implements Iterable<Reference> {
	 ...
	 * @param file
	 */
	public void csvExport(String prefix,boolean withLinks){
		...
		for(Reference r:references){
			...
			datanodes.add(row);
			if(withLinks){for(Reference rc:r.citing){String[] edge = {rc.scholarID,r.scholarID};dataedges.add(edge);}}
		}
		CSVWriter.write(prefix+".csv", datanodes, ";", "\"");
		if(withLinks){CSVWriter.write(prefix+"_links.csv", dataedges, ";", "\"");}
	}
